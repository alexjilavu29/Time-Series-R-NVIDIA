...
Absolventi_irf <- irf(Model1VAR, impulse = 'Unitati', response = 'Absolventi', n.ahead= 10, boot = TRUE)
plot(Absolventi_irf, ylab = 'Absolventi', main = 'Unitati shock to Absolventi')
...
date_norm<-data.frame(absolventi_norm,unitati_norm,biblioteci_norm)

# Selectarea lagului 
lagselect <- VARselect(date_norm, lag.max = 12, type = 'const') #lag=12 - date anuale
lagselect$selection 
# 6 laguri conform AIC, HQ si SC
# Pentru a testa Johansen avem nevoie de laguri selectate - 1 => 6 - 1 = 5

# Testul Johansen - metoda Trace
ctest1 <- ca.jo(date_norm, type = 'trace', ecdet = 'const',K=5)
summary(ctest1) 
...
# Testul Johansen - metoda valorilor proprii maxime
ctest2 <- ca.jo(date_norm, type = 'eigen', ecdet = 'const',K=5)
summary(ctest2)
# r = 0 test > val critice => avem cel putin o relatie de cointegrare
# r <= 1 test > val critice => avem 2 relatii de cointegrare
# r <= 2 test < val critice = > avem cel mult 2 relatii de cointegrare
...
modelVar <- VAR(date_norm, p = 5, type = 'const', season = NULL, exog = NULL)
GrangerAbsolventi <- causality(modelVar, cause = 'absolventi_norm' )
GrangerAbsolventi
GrangerUnitati <- causality(modelVar, cause = 'unitati_norm')
GrangerUnitati
GrangerBiblioteci <- causality(modelVar, cause = 'biblioteci_norm')
GrangerBiblioteci
...
# Functia de raspuns la impuls
Absolventi_irf <- irf(Model1VAR, impulse = 'unitati_norm', response = 'absolventi_norm', n.ahead= 10, boot = TRUE)
plot(Absolventi_irf, ylab = 'absolventi_norm', main = 'Unitati shock to Absolventi')
Absolventi_irf <- irf(Model1VAR, impulse = 'biblioteci_norm', response = 'absolventi_norm', n.ahead= 10, boot = TRUE)
plot(Absolventi_irf, ylab = 'Absolventi', main = 'Bibioteci shock to Absolventi')
...
plot(forecast, name = 'absolventi_norm')
plot(forecast, name = 'unitati_norm')
plot(forecast, name = 'biblioteci_norm')

fanchart(forecast, name = 'absolventi_norm')
fanchart(forecast, name = 'unitati_norm')
fanchart(forecast, name = 'biblioteci_norm')
...
# Testul calculeaza o regresie simpla intre cele doua variabile si testeaza daca reziduurile
# sunt sau nu stationare
coint.test(y = date_norm$absolventi_norm ,X = date_norm$unitati_norm,d = 1) 
# ne uitam la p-value de la no trend si liniar trend
# 0.0942 si 0.1 > 0.05 merge => respingem nula => serii cointegrate
coint.test(y = absolventi_unitati$Unitati,X = absolventi_unitati$Absolventi,d = 1)
